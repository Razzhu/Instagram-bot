#!/usr/bin/env python3
"""
Instagram Group Bot - FINAL: Gemini Key Fix + New Session ID
"""
...
if not GEMINI_API_KEY:
    GEMINI_API_KEY = "AQ.Ab8RN6LAtX8F1t4BLwUtP0Z0mLOGk568IPJQSOgyaYb9FnzcKg"
...
# ============ GEMINI (Using x-goog-api-key header) ============
...
# ============ GEMINI FUNCTIONS (Using x-goog-api-key header) ============
...
def gemini_joke(api_key):
    return gemini_generate("Give a SHORT funny joke in HINDI/HINGLISH (max 20 words). Use emojis.", api_key)

def gemini_roast(target, api_key):
    return gemini_generate(f"Give a SHORT funny savage roast in HINDI/HINGLISH for @{target} (max 15 words). Use emojis.", api_key)

def gemini_compliment(target, api_key):
    return gemini_generate(f"Give a SHORT funny compliment in HINDI/HINGLISH for @{target} (max 15 words). Use emojis.", api_key)

def gemini_fact(api_key):
    return gemini_generate("Give one SHORT interesting fact in HINDI/HINGLISH (max 20 words). Use emojis.", api_key)

def gemini_love(name1, name2, percentage, api_key):
    return gemini_generate(f"Love percentage between {name1} and {name2} is {percentage}%. Give SHORT funny/savage reply in HINDI/HINGLISH (max 15 words). Use emojis.", api_key)

def gemini_trivia(api_key):
    return gemini_generate("Generate ONE trivia question in HINDI/HINGLISH with 4 options.\nFormat:\nQ: [question]\nA) [option1]\nB) [option2]\nC) [option3]\nD) [option4]\nAnswer: [letter]", api_key)

def gemini_natural_reply(message, username, api_key):
    return gemini_generate(f"User @{username} said: '{message}'. Give a SHORT funny/roast/savage/casual reply in HINDI/HINGLISH (max 20 words). Be playful, friendly, and human-like. Use emojis.", api_key)

def gemini_mention_reply(message, username, api_key):
    return gemini_generate(f"User @{username} mentioned the bot and said: '{message}'. Reply casually like a human friend in HINDI/HINGLISH (max 20 words). Be playful, funny, and natural. Use emojis.", api_key)
...
"""
            ...
            if self.gemini_api_key:
                for _ in range(5):
                    q = gemini_trivia(self.gemini_api_key)
                    if q:
                        questions.append(q)
            if len(questions) < 5:
            ...
            if self.gemini_api_key:
                msg = gemini_roast(target, self.gemini_api_key)
            ...
            target = args[0].replace('@', '')
            if self.gemini_api_key:
                msg = gemini_compliment(target, self.gemini_api_key)
            ...
            if self.gemini_api_key:
                msg = gemini_joke(self.gemini_api_key)
            ...
            if self.gemini_api_key:
                msg = gemini_fact(self.gemini_api_key)
            ...
            if self.gemini_api_key:
                msg = gemini_love(name1, name2, percentage, self.gemini_api_key)
            ...
            for thread in threads:
                ...
                try:
                    ...
                    for msg in detail.messages:
                        ...
                        # ============================================================
                        if not msg.text.startswith('.') and random.random() < NATURAL_REPLY_CHANCE and self.gemini_api_key:
                            read_message_like_human(msg.text)

                            if f"@{self.username}".lower() in msg.text.lower():
                                reply = gemini_mention_reply(msg.text, username, self.gemini_api_key)
                            else:
                                reply = gemini_natural_reply(msg.text, username, self.gemini_api_key)
