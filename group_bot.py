...
if not SESSION_ID:
    SESSION_ID = "23395895704:NM9njVq7dxC0V1:16:AYifDVhyF75zUaFJCDzet51-QwlSoszJMs9HLGQSwg"
...
def groq_mention_reply(message, username, api_key):
    return groq_generate(
        f"Reply to @{username} in ONE SHORT SENTENCE (5-10 words) in HINDI/HINGLISH. "
        f"Be playful, rude, and funny. Reply DIRECTLY with the message only. "
        f"Do NOT explain, do NOT add reasoning. Message: '{message}'",
        api_key
    )
...
class InstagramGroupBot:
    ...
    def report_user(self, thread_id, target_username, admin_username, reason="spam"):
        """Report a user to Instagram with specific reason"""
        try:
            print(f"📢 Admin @{admin_username} reporting @{target_username} for: {reason}")
            ...
            url = f"https://www.instagram.com/api/v1/users/{target_id}/flag/"
            data = {
                "reason": reason,
                "source": "group",
                "target_id": target_id
            ...
            if response.status_code == 200:
                print(f"✅ Reported @{target_username} for: {reason}")
...
.report count @user - Report user (1-10 times) 🚨
"""
        ...
        if not self.is_admin(username):
            ...
            report_reasons = [
                "spam", "harassment", "inappropriate", "nudity",
                "hate_speech", "scam", "fake_account", "terrorism", "abuse"
            ]
            ...
            for i in range(count):
                ...
                success, msg = self.report_user(thread_id, target, username, reason)
                if success:
                    success_count += 1
                else:
                    failed_count += 1
            ...
            if success_count > 0 and failed_count == 0:
                ...
                self.send_message(thread_id, f"⚠️ Reported @{target} {success_count} times, {failed_count} failed!")
            else:
                self.send_message(thread_id, f"❌ Failed to report @{target}")
            return
