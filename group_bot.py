#!/usr/bin/env python3
"""
Instagram Group Bot - FIXED MESSAGE READING
"""
...
MAX_COMMANDS_PER_MINUTE = int(os.environ.get("MAX_COMMANDS_PER_MINUTE", 20))
...
class InstagramGroupBot:
    def __init__(self):
        ...
        if not SESSION_ID:
            ...
            print("📌 Please set INSTAGRAM_SESSION_ID environment variable")
            sys.stdout.flush()
            sys.exit(1)
...
"""
    ...
    def check_thread_messages(self, thread_id):
        ...
        try:
            ...
            thread = self.safe_api_call(self.cl.direct_thread, thread_id)
            if not thread:
                print(f"⚠️ Thread {thread_id} returned None")
                sys.stdout.flush()
                return
            ...
            print(f"📬 Thread {thread_id} has {len(thread.messages)} messages")
            sys.stdout.flush()
            ...
            if thread_id not in self.last_message_time:
                print(f"📌 First time checking thread {thread_id}, setting timestamp")
                self.last_message_time[thread_id] = latest_time
                sys.stdout.flush()
                return
            
            print(f"📌 Last seen: {self.last_message_time[thread_id]}, Latest: {latest_time}")
            sys.stdout.flush()
            ...
            if new_messages > 0:
                print(f"✅ Processed {new_messages} new messages in thread {thread_id}")
            else:
        ...
        while self.running:
            try:
                ...
                for thread in threads:
                    ...
                    if self.is_dm(thread):
                        print(f"⏭️ Skipping DM: {thread_id}")
                        continue
