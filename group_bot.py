#!/usr/bin/env python3
"""
Instagram Group Bot - COMPLETE ULTIMATE VERSION
- AFK: Admin only
- Welcome new members
- Welcome back users (after 5 min gap) with 3 messages
- Kick/leave messages with "chala gya bhadwa"
- Fastest response time (2.5 seconds)
- Spam: Up to 10,000 messages with 3 second delay
- Spam Stop: Admin can stop spam anytime
- Kick: Fully fixed
"""
...
# Welcome back messages (3 messages sent with 5 second gap)
WELCOME_BACK_MSGS = [
    "Ohh hiie {username}! 👋",
...
MAX_WARNINGS = 3
POLL_INTERVAL = 2.5
MAX_SPAM_COUNT = 10000  # ✅ 10,000 messages
SPAM_DELAY = 3  # ✅ 3 seconds between each spam message
...
class InstagramGroupBot:
    ...
    def run_spam(self, thread_id, count, message, admin_username):
        """Run spam in a separate thread so it can be stopped"""
...
"""
        ...
        if cmd == '/kick':
            ...
            try:
                ...
                # ✅ FIXED: Correct method to remove user from thread
        ...
        # ✅ SPAM COMMAND - Up to 10,000 messages with 3 second delay
        elif cmd == '/spam':
            if not args:
