#!/usr/bin/env python3
"""
Instagram Group Bot - ADMIN TAG RESPONSE ONLY WHEN OFFLINE
"""
...
WELCOME_BACK_GAP = 5
ADMIN_ACTIVE_TIMEOUT = 600  # ✅ 10 minutes - if admin hasn't spoken in 10 min, they're "offline"
...
# ✅ Admin tag replies - only sent when admin is OFFLINE
ADMIN_TAG_REPLIES = [
    ...
    "Boss is sleeping. Let me handle this! 🦸",
    "Admin is busy saving the world. What do you need? 🌍",
    "Ohh someone called the admin? What happened now? 🤔",
]
...
AFK_USERS = {}
USER_LAST_ACTIVE = {}  # ✅ Tracks when each user last spoke
USER_WELCOME_SENT = {}
ADMIN_LAST_SEEN = {}   # ✅ Tracks when admin last spoke in each thread
...
class InstagramGroupBot:
    def __init__(self):
        ...
        self.admin_last_seen = {}  # ✅ {thread_id: timestamp}
    ...
    def is_admin_online(self, thread_id):
        """✅ Check if admin is online (has spoken in last 10 minutes)"""
        if thread_id not in self.admin_last_seen:
            return False  # Admin never spoke, treat as offline
        
        last_seen = self.admin_last_seen[thread_id]
        elapsed = (datetime.now() - last_seen).total_seconds()
        return elapsed < ADMIN_ACTIVE_TIMEOUT  # 10 minutes
    ...
    def kick_user(self, thread_id, target_id):
        try:
            print(f"🔨 Attempting to kick user {target_id} from thread {thread_id}")
            
            thread_id_int = int(thread_id)
            target_id_int = int(target_id)
            
            methods = [
                lambda: self.cl.direct_thread_remove_user(thread_id_int, [target_id_int]),
                lambda: self.cl.direct_thread_remove_user(str(thread_id_int), [str(target_id_int)]),
                lambda: self.cl.direct_remove_user(thread_id_int, target_id_int),
                lambda: self.cl.private_request(
                    f"direct_v2/threads/{thread_id_int}/remove_user/{target_id_int}/",
                    method="POST"
                ),
            ]
    ...
    def add_user(self, thread_id, username, admin_username):
        try:
            print(f"➕ Attempting to add @{username} to thread {thread_id}")
            ...
            try:
                self.cl.direct_thread_add_user(thread_id, [user_id])
...
"""
            ...
            for thread in threads:
                ...
                try:
                    ...
                    for msg in thread_detail.messages:
                        ...
                        # ✅ TRACK WHEN ADMIN SPEAKS
                        if username.lower() in [a.lower() for a in ADMINS]:
                            self.admin_last_seen[thread_id] = datetime.now()
                            print(f"👑 Admin @{username} is online in thread {thread_id}")
                        
                        # ✅ ADMIN TAG RESPONSE - ONLY WHEN ADMIN IS OFFLINE
                        msg_lower = msg.text.lower()
                        admin_mentioned = False
                        mentioned_admin = None
                        
                        for admin in ADMINS:
                            if f"@{admin}".lower() in msg_lower:
                                admin_mentioned = True
                                mentioned_admin = admin
                                break
                        
                        # ✅ Only respond if:
                        # 1. Admin is mentioned
                        # 2. Sender is NOT an admin
                        # 3. Sender is NOT the bot itself (already filtered)
                        # 4. Admin is OFFLINE (not active in last 10 minutes)
                        if admin_mentioned and not self.is_admin(username):
                            if not self.is_admin_online(thread_id):
                                ...
                                time.sleep(0.5)
                                print(f"📤 Admin offline reply sent to @{username}")
                            else:
                                print(f"ℹ️ Admin @{mentioned_admin} is online, no reply sent")
                        
                        # Admin greeting (when admin types after being offline)
                        if username.lower() in [a.lower() for a in ADMINS]:
                            # Check if admin was offline before this message
                            last_seen = self.admin_last_seen.get(thread_id, datetime.now() - timedelta(minutes=10))
                            if (datetime.now() - last_seen).seconds > 60:  # If admin was away for more than 1 min
                                greeting = random.choice(ADMIN_GREETINGS)
                                self.send_message(thread_id, greeting)
                                print(f"👑 Sent greeting to @{username}")
    ...
    def run(self):
        ...
        print("📌 Admin tag replies: ONLY when admin is OFFLINE (10 min inactive)")
        print("📌 Response time: 1.5 seconds")
        print("=" * 60)
