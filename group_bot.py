#!/usr/bin/env python3
"""
ULTIMATE GC COMPANION BOT - WITH MYSTERY SYSTEM
...
- Interactive Mystery Event System
- And much more!
"""
...
class Config:
    ...
    AI_MAX_TOKENS = 100
    AI_TEMPERATURE = 0.9
    GROQ_MODEL = "groq/compound"  # ✅ Updated to working model
...
# ============ MYSTERY SYSTEM IMPORTS ============
from mystery_database import init_mystery_db, get_active_mystery
from mystery_system import MysterySystem, MYSTERY_CONFIG
...
def clean_ai_response(response):
    """
    Clean AI response to remove reasoning, chain-of-thought, and extra text.
    Returns only the actual response content.
    """
    if not response:
        return None
    ...
    patterns = [
        r'^(?:Reasoning:|Thinking:|Let me think:|Let me reason:|I think:|Here\'s my reasoning:|My reasoning:|Let me consider:|I\'ll think about this:)',
        r'^(?:I\'ll respond:|My response:|Response:|Answer:|Reply:)',
        r'^(?:Alright,|Okay,|Sure,|Here\'s my answer:|I would say:|Let me answer:)',
        r'^(?:Step \d+:|First,|Second,|Third,|Finally,)',
        r'^(?:I need to|I should|I will|Let me|I\'m going to)',
    ]
    ...
    for line in lines:
        ...
        # If this is the actual response (not reasoning)
        if skip_next:
            skip_next = False
            # Check if this line looks like the actual response
            if not re.match(r'^(?:Let me|I think|I\'ll|I need)', line, re.IGNORECASE):
    ...
    if cleaned_lines:
        ...
        # If no lines were cleaned, try to extract the last meaningful part
        result = response
    
    # Additional cleaning
    # Remove any remaining "Response:" prefixes
    result = re.sub(r'^(?:Response:|Answer:|Reply:|AI:|Bot:)', '', result, flags=re.IGNORECASE)
...
def groq_generate(prompt, api_key, max_tokens=Config.AI_MAX_TOKENS, temperature=Config.AI_TEMPERATURE):
    """Generate a response using Groq API with error handling and response cleaning."""
    if not api_key:
        return None
    ...
    try:
        ...
        # Add system message to prevent reasoning
        messages = [
            {
                "role": "system",
                "content": "You are a helpful assistant. Provide ONLY the direct response without any reasoning, chain-of-thought, or explanations. Do not include phrases like 'Let me think', 'I think', 'Here's my reasoning', or any step-by-step thinking. Just give the answer directly."
            },
            {
                "role": "user",
                "content": prompt
        ...
        data = {
            "model": Config.GROQ_MODEL,  # ✅ Using groq/compound from config
            "messages": messages,
        ...
        print(f"[AI] Sending request to Groq with model: {data['model']}")
        ...
        if response.status_code == 200:
            ...
            # If cleaning removed everything, try to use the original
            if not cleaned_reply or len(cleaned_reply) < 2:
                return reply
            
            print(f"[AI] Cleaned response: {cleaned_reply[:50]}...")
        ...
        print("[ERROR] Groq connection error")
        return None
...
def groq_roast(target, api_key):
    """Generate a roast using Groq."""
    prompt = f"Give a SHORT savage roast for @{target} in HINDI/HINGLISH (5-10 words). Use emojis. Only respond with the roast, no other text."
    return groq_generate(prompt, api_key)

def groq_joke(api_key):
    """Generate a joke using Groq."""
    prompt = "Tell a SHORT funny joke in HINDI/HINGLISH (5-10 words). Use emojis. Only respond with the joke, no other text."
    return groq_generate(prompt, api_key)

def groq_fact(api_key):
    """Generate a fact using Groq."""
    prompt = "Give a SHORT interesting fact in HINDI/HINGLISH (5-10 words). Use emojis. Only respond with the fact, no other text."
    return groq_generate(prompt, api_key)

def groq_ask_public(question, api_key):
    """Generate a funny response to a public question."""
    prompt = f"Reply to this question in a funny way in HINDI/HINGLISH (5-10 words): '{question}'. Only respond with the answer, no other text."
    return groq_generate(prompt, api_key)

def groq_admin(question, api_key):
    """Generate an admin response to a question."""
    prompt = f"Answer this question in HINDI/HINGLISH. Be helpful, informative, and complete: '{question}'. Only respond with the answer, no other text."
    return groq_generate(prompt, api_key, max_tokens=200)
...
- ONLY respond with your message, no other text
...
ONLY output the question in this format, no other text.
...
Answer: B"""
    ...
    return groq_generate(prompt, api_key, max_tokens=200, temperature=0.7)
...
class AIRouter:
    ...
    def should_reply(self, thread_id, user_id, message, bot_username, user_count=0):
        ...
        if self.cl and user_id == self.cl.user_id:
...
class InstagramGroupBot:
    def __init__(self):
        ...
        self.mystery_system = MysterySystem(self, self.groq_api_key)
        print("✅ Mystery system initialized!")
    ...
    def _handle_mystery_message(self, thread_id, user_id, username, message):
        """Handle mystery system messages"""
        # Skip if message is empty or too short
        if not message or len(message.strip()) < 2:
            return
        
        # Check if a mystery should start
        members_count = len(self.known_members.get(thread_id, set()))
        
        if self.mystery_system.should_start_mystery(thread_id, members_count):
            members = self._get_thread_members(thread_id)
            result = self.mystery_system.start_mystery(thread_id, members, difficulty="hard")
            if result:
                self.send_message(thread_id, result)
        
        # Process natural language mystery queries
        response = self.mystery_system.on_message(thread_id, user_id, username, message)
        if response:
            self.send_message(thread_id, response)

    def _get_thread_members(self, thread_id):
        """Get list of member usernames in a thread"""
        members = []
        try:
            thread = self.cl.direct_thread(thread_id)
            for user in thread.users:
                if user.pk != self.user_id:
                    username = self.get_username_cached(user.pk)
                    if username:
                        members.append(username)
        except Exception as e:
            print(f"⚠️ Error getting thread members: {e}")
        return members
    ...
    def handle_command(self, thread_id, user_id, username, command):
        ...
        # ============ MYSTERY COMMAND ============
        if cmd == '.mystery':
            result = self.mystery_system.handle_mystery_command(
                thread_id, user_id, username, command, args
            )
            if result:
                self.send_message(thread_id, result)
...
🕵️ MYSTERY SYSTEM:
.mystery start - Start a mystery (admin)
.mystery status - Check current mystery
.mystery investigate - Search for clues
.mystery suspects - List suspects
.mystery evidence - View evidence board
.mystery timeline - Show timeline
...
.mystery leaderboard - View mystery rankings
.mystery stats - View your mystery stats
.mystery stop - Stop mystery (admin)
...
"""
            ...
            for thread in threads:
                ...
                try:
                    ...
                    for msg in detail.messages:
                        ...
                        # ============ MYSTERY SYSTEM HANDLING ============
                        # Check for mystery auto-start and natural queries (only if not a command)
                        if not msg.text.startswith('.'):
                            self._handle_mystery_message(thread_id, msg.user_id, username, msg.text)
    ...
    def run(self):
        ...
        print("   ✅ Clean AI responses (no reasoning)")
        print("   ✅ Interactive Mystery System")
        print("   ✅ Groq Model: groq/compound")
        print("=" * 60)
