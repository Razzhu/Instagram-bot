#!/usr/bin/env python3
"""
Instagram Group Bot - KICK/ADD FIXED, NO SELF-WELCOME
"""
...
class InstagramGroupBot:
    ...
    def send_welcome_back(self, thread_id, username):
        # ✅ DON'T send welcome back to bot itself
        if username == self.username:
            print(f"⚠️ Skipping welcome-back to bot itself ({username})")
            return
    ...
    def kick_user(self, thread_id, target_id):
        """Kick user using the correct instagrapi method"""
        try:
            print(f"🔨 Attempting to kick user {target_id}")
            
            # Convert to int
            thread_id_int = int(thread_id)
            target_id_int = int(target_id)
            
            # ✅ CORRECT METHOD: direct_thread_remove_user
            result = self.cl.direct_thread_remove_user(thread_id_int, [target_id_int])
            ...
            try:
                ...
                print(f"✅ Kick successful via API!")
                return True
            except Exception as e2:
                print(f"❌ Alternative kick failed: {e2}")
                return False
    
    # ✅ FIXED ADD FUNCTION
    def add_user(self, thread_id, username, admin_username):
        """Add user using the correct instagrapi method"""
        try:
            ...
            # ✅ CORRECT METHOD: direct_thread_add_user
            ...
            try:
                self.cl.private_request(
                    f"direct_v2/threads/{thread_id}/add_user/{user_id}/",
                    method="POST"
                )
                print(f"✅ Added @{username} via API!")
                return True, f"✅ @{username} added to the group by @{admin_username}!"
            except Exception as e2:
                print(f"❌ Alternative add failed: {e2}")
...
"""
            ...
            for thread in threads:
                ...
                try:
                    ...
                    for msg in thread_detail.messages:
                        ...
                        # ✅ WELCOME BACK - Skip if sender is the bot
                        ...
                        if username.lower() in [a.lower() for a in ADMINS]:
                            ...
                            print(f"👑 Admin @{username} is online")
    ...
    def run(self):
        ...
        print("📌 Admin tag replies: ONLY when admin is OFFLINE")
        print("📌 No self-welcome: Bot won't welcome itself")
        print("=" * 60)
