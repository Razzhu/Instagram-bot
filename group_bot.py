#!/usr/bin/env python3
"""
Instagram Group Bot - DEBUG KICK
"""
...
class InstagramGroupBot:
    ...
    # ✅ KICK USING DIRECT API - WITH FULL DEBUG
    def kick_user(self, thread_id, target_id):
        """Kick user with full debug logging"""
        try:
            print("=" * 60)
            print(f"🔨 KICK ATTEMPT")
            ...
            print(f"   URL: direct_v2/threads/{thread_id_str}/remove_user/{target_id_str}/")
            
            # ✅ DIRECT API CALL
            response = self.cl.private_request(
                f"direct_v2/threads/{thread_id_str}/remove_user/{target_id_str}/",
                method="POST"
            )
    ...
    # ✅ ADD USING DIRECT API - WITH FULL DEBUG
...
"""
        ...
        if cmd == '/kick':
            ...
            try:
                ...
                if not target_id:
                    error_msg = f"❌ User @{target} not found"
                    print(f"❌ {error_msg}")
                    self.send_message(thread_id, error_msg)
                    return
                ...
                if self.kick_user(thread_id, target_id):
                    ...
                    print(f"✅ Kick successful for @{target}")
                else:
                    error_msg = f"❌ Failed to kick @{target} - see logs for details"
                    print(f"❌ {error_msg}")
                    self.send_message(thread_id, error_msg)
            ...
            for thread in threads:
                ...
                try:
                    ...
                    for msg in thread_detail.messages:
                        ...
                        # Admin tag response (only when offline)
    ...
    def run(self):
        ...
        print("📌 KICK DEBUG: Full logs enabled")
        print("=" * 60)
