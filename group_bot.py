#!/usr/bin/env python3
"""
Instagram Group Bot - ANTI-DETECTION VERSION
...
MAX_COMMANDS_PER_MINUTE = 30  # ✅ Reduced to avoid detection
MAX_WARNINGS = 3
POLL_INTERVAL = 3.0  # ✅ Increased to avoid detection
MAX_SPAM_COUNT = 10000
SPAM_DELAY = 4  # ✅ Increased to avoid detection
...
class InstagramGroupBot:
    ...
    def send_welcome_back(self, thread_id, username):
        ...
        for msg_template in WELCOME_BACK_MSGS:
            ...
            random_delay(2.0, 4.0)  # ✅ Human-like pause between messages
    ...
    def run_spam(self, thread_id, count, message, admin_username):
        ...
        try:
            for i in range(count):
                ...
                random_delay(3.0, 5.0)  # ✅ Longer delay to avoid detection
            else:
    ...
    # ✅ KICK WITH ANTI-DETECTION
    def kick_user(self, thread_id, target_id):
        """Kick user with anti-detection measures"""
        try:
            print(f"\n{'='*60}")
            print(f"🔨 KICK USER")
...
"""
    ...
    def run(self):
        ...
        print("   ✅ Random device fingerprints")
        print("   ✅ Request jitter")
        print("   ✅ Reduced polling frequency")
        print("=" * 60)
        ...
        while self.running:
            try:
                self.check_messages()
                # ✅ Random polling interval to avoid detection
                poll_delay = random.uniform(2.0, 4.0)
