#!/usr/bin/env python3
"""
Instagram Group Bot - GROQ COMPOUND (Confirmed Working)
"""
...
def groq_generate(prompt, api_key):
    ...
    try:
        ...
        data = {
            "model": "groq/compound",  # ✅ Confirmed working
...
def groq_joke(api_key):
    return groq_generate("Give a SHORT funny joke in HINDI/HINGLISH (max 20 words). Use emojis.", api_key)

def groq_roast(target, api_key):
    return groq_generate(f"Give a SHORT funny savage roast in HINDI/HINGLISH for @{target} (max 15 words). Use emojis.", api_key)

def groq_compliment(target, api_key):
    return groq_generate(f"Give a SHORT funny compliment in HINDI/HINGLISH for @{target} (max 15 words). Use emojis.", api_key)

def groq_fact(api_key):
    return groq_generate("Give one SHORT interesting fact in HINDI/HINGLISH (max 20 words). Use emojis.", api_key)

def groq_love(name1, name2, percentage, api_key):
    return groq_generate(f"Love percentage between {name1} and {name2} is {percentage}%. Give SHORT funny/savage reply in HINDI/HINGLISH (max 15 words). Use emojis.", api_key)

def groq_trivia(api_key):
    return groq_generate("Generate ONE trivia question in HINDI/HINGLISH with 4 options.\nFormat:\nQ: [question]\nA) [option1]\nB) [option2]\nC) [option3]\nD) [option4]\nAnswer: [letter]", api_key)

def groq_natural_reply(message, username, api_key):
    return groq_generate(f"User @{username} said: '{message}'. Give a SHORT funny/roast/savage/casual reply in HINDI/HINGLISH (max 20 words). Be playful, friendly, and human-like. Use emojis.", api_key)

def groq_mention_reply(message, username, api_key):
    return groq_generate(f"User @{username} mentioned the bot and said: '{message}'. Reply casually like a human friend in HINDI/HINGLISH (max 20 words). Be playful, funny, and natural. Use emojis.", api_key)
...
class InstagramGroupBot:
    def __init__(self):
        ...
        if self.groq_api_key:
            print("✅ Groq API key loaded! (groq/compound)")
        else:
...
"""
            ...
            if self.groq_api_key:
                msg = groq_love(name1, name2, percentage, self.groq_api_key)
    ...
    def run(self):
        ...
        print("   ✅ Fresh jokes, roasts, compliments from Groq")
        print("   ✅ Fallback only if Groq fails")
        print("=" * 60)
