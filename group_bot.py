#!/usr/bin/env python3
"""
Instagram Group Bot - ANTI-DETECTION (20% Natural Reply)
"""
...
COMMAND_COOLDOWN = {
    ...
    '.fact': 10, '.joke': 10, '.meme': 10, '.quote': 10,
    '.love': 10, '.score': 5, '.leaderboard': 15,
    '.help': 15, '.rules': 15,
    '.kick': 60, '.warn': 60, '.add': 60,
    '.mute': 60, '.unmute': 60, '.clearwarn': 60,
    '.slowmode': 60, '.spam': 300, '.stopspam': 10,
...
# ✅ NATURAL REPLY CHANCE = 20%
NATURAL_REPLY_CHANCE = 0.20
...
def groq_natural_reply(message, username, api_key):
    return groq_generate(
        ...
        f"Be savage, rude, funny, and casual. Message: '{message}'",
        api_key
    )
...
def groq_mention_reply(message, username, api_key):
    return groq_generate(
        f"Reply to @{username} in ONE SHORT SENTENCE (5-10 words) in HINDI/HINGLISH. "
        f"Be playful, rude, and funny. Message: '{message}'",
        api_key
    )

def groq_joke(api_key):
    return groq_generate("Tell a SHORT joke in HINDI/HINGLISH (5-10 words). Use emojis.", api_key)
...
# ============ FALLBACK ============
FALLBACK_JOKES = ["एक आदमी ने डॉक्टर से कहा: मुझे हर रात बुरे सपने आते हैं। डॉक्टर बोला: क्या सपने आते हैं? आदमी बोला: सपने में मुझे नींद नहीं आती! 😂", "टीचर: तुम्हारी कॉपी में तो दीमक लग गई है! स्टूडेंट: सर, वो मेरी क्रिएटिविटी है! 💀"]
FALLBACK_ROASTS = ["तू हर जगह है जैसे WiFi, पर काम किसी काम का नहीं! 📶", "तेरी सोच इतनी गहरी है जितनी चाय की प्लेट! ☕"]
FALLBACK_COMPLIMENTS = ["तू तो लगता है जैसे सुबह की चाय — हर किसी को भाती है! ☕", "तू वो दोस्त है जिसके बिना ग्रुप अधूरा है! ❤️"]
FALLBACK_FACTS = ["ऑक्टोपस के 3 दिल होते हैं! 💙", "केला एक बेरी है! 🍌"]
FALLBACK_LOVE = ["तुम दोनों एक दूसरे के लिए बने हो! ❤️", "ये प्यार है... या फिर मजाक? 🤔"]
FALLBACK_TRIVIA = [{"q": "फ्रांस की राजधानी क्या है?", "a": "पेरिस"}, {"q": "2+2 क्या होता है?", "a": "4"}]
...
**🤖 AI Mode:**
Bot replies naturally in Hindi (20% chance)
"""
    ...
    def run(self):
        print("\n" + "=" * 60)
        print("🤖 GROUP BOT RUNNING (20% Natural Reply)")
        print(f"👤 Bot: @{self.username}")
        print(f"👑 Admins: {', '.join(ADMINS)}")
        print(f"📊 Monitoring: {getattr(self, 'group_count', 1)} group(s)")
        print("=" * 60)
        print("🤖 Features:")
        print("   ✅ 20% natural reply chance")
        print("   ✅ Savage/Rude/Funny tone")
        print("   ✅ Hindi/Hinglish")
        print("   ✅ Anti-detection (5-10s polling)")
        print("=" * 60)
